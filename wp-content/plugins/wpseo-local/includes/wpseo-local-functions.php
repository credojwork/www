	...
	*/
	...
	foreach ( $locations as $location ) {
		...
		if ( false == $atts['hide_name'] ) {
			$output .= $tag_title_open . ( ( $atts['from_sl'] ) ? '<a href="' . esc_url( $location['business_url'] ) . '">' : '' ) . '<span class="wpseo-business-name" itemprop="name">' . esc_html( $location['business_name'] ) . '</span>' . ( ( $atts['from_sl'] ) ? '</a>' : '' ) . $tag_title_close;
		}
		else {
	...
	foreach ( $locations as $location ) {
		...
		foreach ( $days as $key => $day ) {
			...
			if ( $value_from != 'closed' && $value_to != 'closed' ) {
				$output_time .= '<span ' . ( ( $show_schema ) ? 'itemprop="openingHours"' : '' ) . ' content="' . $day_abbr . ' ' . $value_from . '-' . $value_to . '">' . $value_from_formatted . ' - ' . $value_to_formatted . '</span>';
			}
			else {
			...
			if ( $multiple_opening_hours ) {
				if ( $value_from != 'closed' && $value_to != 'closed' && $value_second_from != 'closed' && $value_second_to != 'closed' ) {
					...
					$output_time .= '<span ' . ( ( $show_schema ) ? 'itemprop="openingHours"' : '' ) . ' content="' . $day_abbr . ' ' . $value_second_from . '-' . $value_second_to . '">' . $value_second_from_formatted . ' - ' . $value_second_to_formatted . '</span>';
				}
				else {
					$output_time .= '';
				}
...
function wpseo_enqueue_geocoder() {
	...
	if ( $wpseo_enqueue_geocoder ) {
		...
		$script_name = 'wp-seo-local-frontend-490.min.js';
		if ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) {
			$script_name = 'wp-seo-local-frontend-490.js';
		}

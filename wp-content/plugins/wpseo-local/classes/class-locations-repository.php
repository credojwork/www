...
if ( ! class_exists( 'WPSEO_Local_Locations_Repository' ) ) {
	...
	class WPSEO_Local_Locations_Repository {
		...
		public function cb_options_url( $options ) {
			return WPSEO_Sitemaps_Router::get_base_url( '' );
		}
		 ...
		 */
		public function cb_options_logo( $options ) {
			if ( isset( $options['company_logo'] ) && intval( $options['company_logo'] ) ) {
				return $options['company_logo'];
			}

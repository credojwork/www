	...
	class WPSEO_Local_Metaboxes {
		...
		function add_mce_popup() {
            ...
            <script>
                function WPSEO_InsertMap() {
                    ...
                    window.send_to_editor("[wpseo_map " + id + term + center + " width=\"" + width + "\" height=\"" + height + "\" zoom=\"" + zoom + "\" map_style=\"" + map_style + "\"" + scrollable + draggable + marker_clustering + show_route + show_state + show_country + show_url + show_email + show_category_filter + default_show_infowindow + "]");
                }
                ...
                function WPSEO_InsertStorelocator() {
                    ...
                    window.send_to_editor("[wpseo_storelocator " + show_map + scrollable + draggable + marker_clustering + show_radius + show_nearest_suggestion + radius + show_filter + " map_style=\"" + map_style + "\"" + oneline + show_state + show_country + show_phone + show_phone_2 + show_fax + show_email + show_url + show_opening_hours + hide_closed + show_category_filter + "]");
                }
		...
		function enqueue_scripts() {
			...
			if ( 'wpseo_locations' == get_post_type() ) {
				wp_enqueue_script( 'wp-seo-local-seo-locations', plugins_url( '../js/wp-seo-local-analysis-locations-450' . WPSEO_CSSJS_SUFFIX . '.js', __FILE__ ), array(), WPSEO_VERSION, true );

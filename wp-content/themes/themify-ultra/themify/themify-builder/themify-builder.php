...
if (!function_exists('themify_manage_builder')) {
    ...
    function themify_manage_builder($data = array()) {
        ...
        foreach ($modules as $m) {
            ...
            $output .= '<p><span><input id="' . esc_attr('builder_module_' . $m['id']) . '" type="checkbox" name="' . esc_attr($exclude) . '" value="1" ' . $checked . '/> <label for="' . esc_attr('builder_module_' . $m['id']) . '">' . wp_kses_post(sprintf(__('Exclude %s module', 'themify'), $m['name'])) . '</label></span></p>';
        }
...
if (!function_exists('themify_manage_builder_active')) {
    ...
    function themify_manage_builder_active($data = array()) {
        ...
        $output .= sprintf('<p><span class="label">%s</span><select id="%s" name="%s">%s</select>%s</p>', esc_html__('Themify Builder:', 'themify'), esc_attr($pre . 'is_active'), esc_attr($pre . 'is_active'), themify_options_module($options, $pre . 'is_active'), sprintf('<small class="pushlabel" data-show-if-element="[name=setting-page_builder_is_active]" data-show-if-value="disable">%s</small>'
...
if (!function_exists('themify_manage_builder_animation')) {
    ...
    function themify_manage_builder_animation($data = array()) {
        ...
        $output = '';
        $output .= sprintf('<p><label for="%s" class="label">%s</label><select id="%s" name="%s">%s</select></p>', esc_attr($pre . 'appearance'), esc_html__('Appearance Animation', 'themify'), esc_attr($pre . 'appearance'), esc_attr($pre . 'appearance'), themify_options_module($options, $pre . 'appearance')
        );
        $output .= sprintf('<p><label for="%s" class="label">%s</label><select id="%s" name="%s">%s</select></p>', esc_attr($pre . 'parallax_bg'), esc_html__('Parallax Background', 'themify'), esc_attr($pre . 'parallax_bg'), esc_attr($pre . 'parallax_bg'), themify_options_module($options, $pre . 'parallax_bg')
        );
        $output .= sprintf('<p><label for="%s" class="label">%s</label><select id="%s" name="%s">%s</select></p>', esc_attr($pre . 'parallax_scroll'), esc_html__('Parallax Scrolling', 'themify'), esc_attr($pre . 'parallax_scroll'), esc_attr($pre . 'parallax_scroll'), themify_options_module($options, $pre . 'parallax_scroll', true, 'mobile')
        );

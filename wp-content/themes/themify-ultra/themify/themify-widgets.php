...
class Themify_Google_Maps extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ($instance['map_display_type'] == 'static') : ?>
            ...
            $args .= '&size=' . ( isset( $instance['width'] ) ? $instance['width'] : '500' ) . 'x' . ( isset( $instance['height'] ) ? $instance['height'] : '300' );
            ?>
        ...
        <?php else : ?>
            ...
            if (!empty($instance['address_map']) || !empty($instance['latlong_map'])) {
                ...
                ?>
                <div data-map="<?php echo esc_attr( json_encode( $data ) ); ?>" class="themify_map map-container"  style="<?php echo esc_attr($style); ?>"  data-info-window="<?php echo esc_attr($instance['info_window_map']); ?>" data-reverse-geocoding="<?php echo ( empty($instance['address_map']) && !empty($instance['latlong_map']) ) ? true : false; ?>"></div>
            <?php } ?>
        <?php
        endif;
		echo '</div>';

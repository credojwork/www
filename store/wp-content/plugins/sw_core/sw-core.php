 ...
 extract( shortcode_atts( array(
  ...
  'el_class' => ''
	...
	if( function_exists( 'onemall_options' ) ){
		$onemall_menu_item 	= ( onemall_options()->getCpanelValue( 'menu_number_item' ) )  ? onemall_options()->getCpanelValue( 'menu_number_item' )  : 9;
		$onemall_mmenu_item	= ( onemall_options()->getCpanelValue( 'mmenu_number_item' ) ) ? onemall_options()->getCpanelValue( 'mmenu_number_item' ) : 6;
		$onemall_more_text 	= ( onemall_options()->getCpanelValue( 'menu_more_text' ) )	 ? onemall_options()->getCpanelValue( 'menu_more_text' )		: esc_html__( 'See More', 'sw_core' );
		$onemall_less_text 	= onemall_options()->getCpanelValue( 'menu_less_text' )			 ? onemall_options()->getCpanelValue( 'menu_less_text' )		: esc_html__( 'See Less', 'sw_core' );
		 $attributes .= 'data-number="'.esc_attr( $onemall_menu_item ).'" data-mnumber="'.esc_attr( $onemall_mmenu_item ).'" data-moretext="'.esc_attr( $onemall_more_text ).'" data-lesstext="'.esc_attr( $onemall_less_text ).'"';
	}
	$output = '<div class="vc_wp_custommenu wp_verticle_onemall wpb_content_element ' . $el_class . '">';
	if($title != ''){
...
/*
   ...
   *
  */
  function onemall_change_logo( $atts ){
		extract(shortcode_atts(array(
		'title' => '',
		'colorset' => '',
		'post_status' => 'publish',
		...
		if( function_exists( 'onemall_options' ) ) {
			...
			$site_logo = onemall_options()->getCpanelValue('sitelogo');
			$output   ='';
			$output  .='<div class="ya-logo">';
			$output	 .='<a  href="'.esc_url( home_url( '/' ) ).'">';
						if(onemall_options()->getCpanelValue('sitelogo')){
			$output	 .='<img src="'.esc_url( $onemall_logo ).'" alt="logo"/>';
						}else{
  ...
  add_shortcode('logo_footer','onemall_change_logo');

 /***********************
 * Topdeal IMG SLIDER
 *
 ***************************/
 function onemall_img_slide($atts){
	extract( shortcode_atts( array(
		'title' => '',
		'ids' => '',
		'fade' =>'true',
...
foreach ( $ids as $attach_id ) :  
    ...
    $html .='<div class="image"><img src="'.esc_url( $linkimg ).'" alt="'.esc_html__('slide show','onemall').'"></div>';
endforeach ;
$html .='</div>';
return $html;
}
 add_shortcode('img_slide','onemall_img_slide');

<?php
class Onemall_Options_multi_field extends Onemall_Options{	
	...
	*/
	function render(){
		...
		if(isset($this->value) && is_array($this->value)){
			foreach($this->value as $k => $value){
				foreach ($this->field['sub_fields'] as $field ) { 
					...
					$class_field = 'Onemall_Options_'.$field['type'];
		...
		echo '<a href="javascript:void(0);" class="onemall-opts-multi-field-add" rel-id="'.esc_attr( $this->field['id'] ).'-table" rel-name="'.$this->args['opt_name'].'['.$this->field['id'].'][]">'.esc_html__('Add More', 'onemall').'</a><br/>';
	...
	*/
	function enqueue(){
		...
		wp_enqueue_script(
			'onemall-opts-field-multi-field-js', 
			ONEMALL_OPTIONS_URL.'fields/multi_field/field_multi_field.js', 
			array('jquery'),
			time(),
			true
		);

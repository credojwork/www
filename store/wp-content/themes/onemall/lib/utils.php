 ...
 * Page titles
 */
function onemall_title() {
	if (is_home()) {
		if (get_option('page_for_posts', true)) {
			echo get_the_title(get_option('page_for_posts', true));
		} else {
			esc_html_e('Latest Posts', 'onemall');
		}
	} elseif (is_archive()) {
		...
		if ($term) {
			...
			printf(__('Daily Archives: %s', 'onemall'), get_the_date());
		} elseif (is_month()) {
			printf(__('Monthly Archives: %s', 'onemall'), get_the_date('F Y'));
		} elseif (is_year()) {
			printf(__('Yearly Archives: %s', 'onemall'), get_the_date('Y'));
		} elseif (is_author()) {
			printf(__('Author Archives: %s', 'onemall'), get_the_author());
		} else {
			single_cat_title();
		}
	} elseif (is_search()) {
		printf(__('Search Results for <small>%s</small>', 'onemall'), get_search_query());
	} elseif (is_404()) {
		esc_html_e('Not Found', 'onemall');
	} else {
		the_title();
	}
}
 ...
 */
function onemall_leadingslashit($string) {
	return '/' . onemall_unleadingslashit($string);
}

function onemall_unleadingslashit($string) {
	return ltrim($string, '/');
}

function onemall_element_empty($element) {
	$element = trim($element);
	return empty($element) ? false : true;
}

function onemall_customize(){
...
*/
class Onemall_Menu_Checkbox extends Walker_Nav_Menu{
...
*/
function onemall_check_wdisplay ($widget_display){
	$widget_display = json_decode(json_encode($widget_display), true);
	$Onemall_Menu_Checkbox = new Onemall_Menu_Checkbox;
					...
					if ( ($key == 'login' && $user_ID) || ($key == 'logout' && !$user_ID) ){
						...
						if (isset($widget_display['post-type'])) {
							...
							foreach ($post_type as $type) {
								...
								if($type!=ONEMALL_PRODUCT_TYPE)
								{
									if(!empty($type) && $type==ONEMALL_PRODUCT_DETAIL_TYPE && is_single() && get_post_type() != 'post'){
										return true;
						...
						if (isset($widget_display['checkbox']['menus'])) {
							...
							foreach ($widget_display['checkbox']['menus'] as $menu_id => $item_ids) {
								...
								if ( $Onemall_Menu_Checkbox->is_menu_item_active($menu_id, $item_ids) ) return true;
							}
						}
					}
				}
		...
		if (isset($widget_display['checkbox'])) {
			...
			if (isset($widget_display['checkbox']['menus'])) {
				...
				foreach ($widget_display['checkbox']['menus'] as $menu_id => $item_ids) {
					...
					if ( $Onemall_Menu_Checkbox->is_menu_item_active($menu_id, $item_ids) ) return false;
				}
			}			
		} else return false ;
	}
...
**  Is active sidebar
*/
function onemall_sidebar_check($index) {
		...
		foreach ($sidebars_widgets[$index] as $i => $id) {
			...
			if ( isset($wp_registered_widgets[$id]) ) {
				...
				if ( isset($instances) && isset($number) ) {
					...
					if ( isset($instance['widget_display']) && onemall_check_wdisplay($instance['widget_display']) == false ) {
...
** Get Social share
*/
function onemall_get_social() {
	global $post;
	
	$social = onemall_options()->getCpanelValue('social_share');	
	
	if ( !$social ) return false;
	ob_start();
?>
	<div class="social-share">
		<div class="title-share"><?php esc_html_e( 'Share','onemall' ) ?></div>
		<div class="wrap-content">
			<a href="http://www.facebook.com/share.php?u=<?php echo get_permalink( $post->ID ); ?>&title=<?php get_the_title( $post->ID ); ?>" onclick="javascript:window.open(this.href,'', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');return false;"><i class="fa fa-facebook"></i></a>
			<a href="http://www.tumblr.com/share?v=3&u=<?php echo get_permalink( $post->ID ); ?>&t=<?php get_the_title( $post->ID ); ?>" onclick="javascript:window.open(this.href,'', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');return false;"><i class="fa fa-tumblr"></i></a>
			<a href="http://twitter.com/home?status=<?php echo get_the_title( $post->ID ); ?>+<?php get_permalink( $post->ID ); ?>" onclick="javascript:window.open(this.href,'', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');return false;"><i class="fa fa-twitter"></i></a>
			...
			<a href="http://pinterest.com/pin/create/bookmarklet/?media=<?php echo wp_get_attachment_url( get_post_thumbnail_id($post->ID) ); ?>&url=<?php echo get_permalink( $post->ID ); ?>&is_video=false&description=<?php get_the_title( $post->ID ); ?>" onclick="javascript:window.open(this.href,'', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');return false;"><i class="fa fa-pinterest-p"></i></a>
		</div>
	</div>
<?php 
	$data = ob_get_clean();
...
function onemall_get_avatar($avatar) {
	$avatar = str_replace("class='avatar", "class='avatar pull-left media-object", $avatar);
	return $avatar;
}
add_filter('get_avatar', 'onemall_get_avatar');


/*
** Check col for sidebar and content product
*/
function onemall_content_product(){ 
	$left_span_class 			= onemall_options()->getCpanelValue('sidebar_left_expand');
	$left_span_md_class 	= onemall_options()->getCpanelValue('sidebar_left_expand_md');
	$left_span_sm_class 	= onemall_options()->getCpanelValue('sidebar_left_expand_sm');
	$right_span_class 		= onemall_options()->getCpanelValue('sidebar_right_expand');
	$right_span_md_class 	= onemall_options()->getCpanelValue('sidebar_right_expand_md');
	$right_span_sm_class 	= onemall_options()->getCpanelValue('sidebar_right_expand_sm');
	$sidebar 							= onemall_options()->getCpanelValue('sidebar_product');
	if( !is_post_type_archive( 'product' ) ){
		$term_id = get_queried_object()->term_id;
		$sidebar = ( get_term_meta( $term_id, 'term_sidebar', true ) != '' ) ? get_term_meta( $term_id, 'term_sidebar', true ) : onemall_options()->getCpanelValue('sidebar_product');
	}
...
*/
function onemall_content_product_detail(){
	$left_span_class 			= onemall_options()->getCpanelValue('sidebar_left_expand');
	$left_span_md_class 	= onemall_options()->getCpanelValue('sidebar_left_expand_md');
	$left_span_sm_class 	= onemall_options()->getCpanelValue('sidebar_left_expand_sm');
	$right_span_class 		= onemall_options()->getCpanelValue('sidebar_right_expand');
	$right_span_md_class 	= onemall_options()->getCpanelValue('sidebar_right_expand_md');
	$right_span_sm_class 	= onemall_options()->getCpanelValue('sidebar_right_expand_sm');
	$sidebar_template 		= onemall_options()->getCpanelValue('sidebar_product_detail');
	
	if( is_singular( 'product' ) ) :
		$sidebar_template = ( get_post_meta( get_the_ID(), 'page_sidebar_layout', true ) != '' ) ? get_post_meta( get_the_ID(), 'page_sidebar_layout', true ) : onemall_options()->getCpanelValue('sidebar_product_detail');
...
function onemall_content_blog(){
	...
	$right_span_md_class 	= onemall_options()->getCpanelValue('sidebar_right_expand_md');
	$right_span_sm_class 	= onemall_options()->getCpanelValue('sidebar_right_expand_sm');
	$sidebar_template 		= onemall_options() -> getCpanelValue('sidebar_blog');
	$sidebar  						= 'left-blog';
	if( is_single() ) :
		$sidebar_template = ( strlen( get_post_meta( get_the_ID(), 'page_sidebar_layout', true ) ) > 0 ) ? get_post_meta( get_the_ID(), 'page_sidebar_layout', true ) : onemall_options()->getCpanelValue('sidebar_blog');
...
** Check sidebar blog
*/
function onemall_sidebar_template(){
	$onemall_sidebar_teplate = onemall_options() -> getCpanelValue('sidebar_blog');
	if( !is_archive() ){
		$onemall_sidebar_teplate = ( get_term_meta( get_queried_object()->term_id, 'term_sidebar', true ) != '' ) ? get_term_meta( get_queried_object()->term_id, 'term_sidebar', true ) : onemall_options()->getCpanelValue('sidebar_blog');
	}	
	if( is_single() ) {
		$onemall_sidebar_teplate = ( get_post_meta( get_the_ID(), 'page_sidebar_layout', true ) != '' ) ? get_post_meta( get_the_ID(), 'page_sidebar_layout', true ) : onemall_options()->getCpanelValue('sidebar_blog');
	}
	return $onemall_sidebar_teplate;
}

/*
** Check col for sidebar and content page
*/
function onemall_content_page(){
	$left_span_class 			= onemall_options()->getCpanelValue('sidebar_left_expand');
	$left_span_md_class 	= onemall_options()->getCpanelValue('sidebar_left_expand_md');
	$left_span_sm_class 	= onemall_options()->getCpanelValue('sidebar_left_expand_sm');
	$right_span_class 		= onemall_options()->getCpanelValue('sidebar_right_expand');
...
** Typography
*/
function onemall_typography_css(){
	$styles = '';
	$page_webfonts  = get_post_meta( get_the_ID(), 'google_webfonts', true );
	$webfont 		= ( $page_webfonts != '' ) ? $page_webfonts : onemall_options()->getCpanelValue( 'google_webfonts' );
	$header_webfont = onemall_options()->getCpanelValue( 'header_tag_font' );
	$menu_webfont 	= onemall_options()->getCpanelValue( 'menu_font' );
	$custom_webfont = onemall_options()->getCpanelValue( 'custom_font' );
	$custom_class 	= onemall_options()->getCpanelValue( 'custom_font_class' );
...
function onemall_typography_css_cache(){ 
		
	/* Custom Css */
	if ( onemall_options()->getCpanelValue('advanced_css') != '' ){
		echo'<style>'. onemall_options()->getCpanelValue( 'advanced_css' ) .'</style>';
	}
	$data = onemall_typography_css();
	echo $data;
}
add_action( 'wp_head', 'onemall_typography_css_cache', 12, 0 );

function onemall_typography_webfonts(){
	$page_google_webfonts = get_post_meta( get_the_ID(), 'google_webfonts', true );
	$webfont 		= ( $page_google_webfonts != '' ) ? $page_google_webfonts : onemall_options()->getCpanelValue('google_webfonts');
	$header_webfont = onemall_options()->getCpanelValue( 'header_tag_font' );
	$menu_webfont 	= onemall_options()->getCpanelValue( 'menu_font' );
	$custom_webfont = onemall_options()->getCpanelValue( 'custom_font' );
	...
	if ( $webfont || $header_webfont || $menu_webfont || $custom_webfont ):
		...
		$webfont_weight	= ( get_post_meta( get_the_ID(), 'webfonts_weight', true ) ) ? get_post_meta( get_the_ID(), 'webfonts_weight', true ) : onemall_options()->getCpanelValue('webfonts_weight');
		$font_weight = '';
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'onemall' ) ) {
...
function onemall_googlefonts_script() {
    wp_enqueue_style( 'onemall-googlefonts', onemall_typography_webfonts(), array(), '1.0.0' );
}
add_action( 'wp_enqueue_scripts', 'onemall_googlefonts_script' );


/* 
** Get video or iframe from content 
*/
function onemall_get_entry_content_asset( $post_id ){
	global $post;
...
function onemall_excerpt($limit) {
...
** Tag cloud size
*/
add_filter( 'widget_tag_cloud_args', 'onemall_tag_clound' );
function onemall_tag_clound($args){
	$args['largest'] = 8;
	return $args;
}
...
*/
if( !is_admin() ){
	add_filter( 'language_attributes', 'onemall_direction', 20 );
	function onemall_direction( $doctype = 'html' ){
		$onemall_direction = onemall_options()->getCpanelValue( 'direction' );
		if ( ( function_exists( 'is_rtl' ) && is_rtl() ) || $onemall_direction == 'rtl' )
			$onemall_attribute[] = 'dir="rtl"';
		( $onemall_direction === 'rtl' ) ? $lang = 'ar' : $lang = get_bloginfo('language');
		if ( $lang ) {
		if ( get_option('html_type') == 'text/html' || $doctype == 'html' )
			$onemall_attribute[] = "lang=\"$lang\"";

		if ( get_option('html_type') != 'text/html' || $doctype == 'xhtml' )
			$onemall_attribute[] = "xml:lang=\"$lang\"";
		}
		$onemall_output = implode(' ', $onemall_attribute);
		return $onemall_output;
	}
}
...
class onemall_Breadcrumbs {
	...
	function breadcrumb( $before = '', $after = '', $display = true ) {
		$options = array('breadcrumbs-home' => esc_html__( 'Home', 'onemall' ), 'breadcrumbs-blog-remove' => false, 'post_types-post-maintax' => '0');
		...
		$links = array(
			array(
				'url'  => get_home_url(),
				'text' => ( isset( $options['breadcrumbs-home'] ) && $options['breadcrumbs-home'] != '' ) ? $options['breadcrumbs-home'] : esc_html__( 'Home', 'onemall' )
			)
		);
		...
		if ( ( $on_front == "page" && is_front_page() ) || ( $on_front == "posts" && is_home() ) ) {
			...
			if ( is_post_type_archive() ) {
				...
				$bc = esc_html__( 'Archives for', 'onemall' );
				...
				$bc = esc_html__( 'You searched for', 'onemall' );
				$links[] = array( 'text' => $bc . ' "' . esc_html( get_search_query() ) . '"' );
			} elseif ( is_404() ) {
				$crumb404 = esc_html__( 'Error 404: Page not found', 'onemall' );
...
global $onemall_breadcrumb;
$onemall_breadcrumb = new onemall_Breadcrumbs();

if ( !function_exists( 'onemall_breadcrumb' ) ) {
	/**
	 ...
	 * @return string
	 */
	function onemall_breadcrumb( $before = '', $after = '', $display = true ) {
		global $onemall_breadcrumb;
		
		/* Turn off Breadcrumb */
		if( onemall_options()->getCpanelValue( 'breadcrumb_active' ) ) :
			$display = false;
		endif;
		return $onemall_breadcrumb->breadcrumb( $before, $after, $display );
	}
}
...
** Footer Adnvanced
*/
add_action( 'wp_footer', 'onemall_footer_advanced' );
function onemall_footer_advanced(){
	/* 
	** Back To Top 
	*/
	if( onemall_options()->getCpanelValue( 'back_active' ) ) :
		echo '<a id="onemall-totop" href="#" ></a>';
	endif;
	
	/* 
	** Popup 
	*/
	if( onemall_options()->getCpanelValue( 'popup_active' ) ) :
		$onemall_content = onemall_options()->getCpanelValue( 'popup_content' );
		$onemall_shortcode = onemall_options()->getCpanelValue( 'popup_form' );
		//$popup_attr = ( onemall_options()->getCpanelValue( 'popup_background' ) != '' ) ? 'style="background: url( '. esc_url( onemall_options()->getCpanelValue( 'popup_background' ) ) .' )"' : '';
?>
		<div id="subscribe_popup" class="subscribe-popup">
			<div class="subscribe-popup-container clearfix">
				<div class="image-newsletter pull-left">
					<img src="<?php echo esc_url( onemall_options()->getCpanelValue( 'popup_background' ) )?>" alt="image-newsletter"/>
				</div>
				<div class="subscribe-content pull-right">
					<?php if( $onemall_content != '' ) : ?>
					<div class="popup-content">
						<?php echo $onemall_content; ?>
					</div>
					<?php endif; ?>
					
					<?php if( $onemall_shortcode != '' ) : ?>
					...
					<div class="subscribe-checkbox">
						<label for="popup_check">
							...
							<?php echo '<span>' . esc_html__( "Don't show this popup again!", "onemall" ) . '</span>'; ?>
						</label>
					</div>
...
?>
	<div class="modal fade" id="login_form" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog block-popup-login">
			<a href="javascript:void(0)" title="<?php esc_attr_e( 'Close', 'onemall' ) ?>" class="close close-login" data-dismiss="modal"><?php esc_html_e( 'Close', 'onemall' ) ?></a>
		<div class="tt_popup_login"><strong><?php esc_html_e('Sign in Or Register', 'onemall'); ?></strong></div>
		<?php get_template_part('woocommerce/myaccount/login-form'); ?>
		<?php 
			if( class_exists( 'APSL_Lite_Class' ) ) :
				echo '<div class="login-line"><span>'. esc_html__( 'Or', 'onemall' ) .'</span></div>';
	...
	<div class="modal fade" id="search_form" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog block-popup-search-form">
			<form role="search" method="get" class="form-search searchform" action="<?php echo esc_url( home_url( '/' ) ); ?>">
				<input type="text" value="<?php if (is_search()) { echo get_search_query(); } ?>" name="s" class="search-query" placeholder="<?php esc_attr_e( 'Enter your keyword...', 'onemall' ) ?>">
				<button type="submit" class=" fa fa-search button-search-pro form-button"></button>
				<a href="javascript:void(0)" title="<?php esc_attr_e( 'Close', 'onemall' ) ?>" class="close close-search" data-dismiss="modal"><?php esc_html_e( 'X', 'onemall' ) ?></a>
			</form>
		</div>
	</div>
<?php 	
...
**/
function onemall_advanced(){	
	$onemall_popup	 		= onemall_options()->getCpanelValue( 'popup_active' );
	$sticky_mobile	 		= onemall_options()->getCpanelValue( 'sticky_mobile' );
	$output  = '';
	$output .= '(function($) {';
	if( !onemall_mobile_check() ) : 
		$sticky_menu 		= onemall_options()->getCpanelValue( 'sticky_menu' );
		$onemall_header_style 	= ( get_post_meta( get_the_ID(), 'page_header_style', true ) != '' ) ? get_post_meta( get_the_ID(), 'page_header_style', true ) : onemall_options()->getCpanelValue('header_style');
		$output_css = '';
		$layout = onemall_options()->getCpanelValue('layout');
		$bg_image = onemall_options()->getCpanelValue('bg_box_img');
		$header_mid = onemall_options()->getCpanelValue('header_mid');
		$bg_header_mid = onemall_options()->getCpanelValue('bg_header_mid');			
		...
		if( $layout == 'boxed' ){
			...
			$output_css .= '}';
			wp_enqueue_style(	'onemall_custom_css',	get_template_directory_uri() . '/css/custom_css.css' );
			wp_add_inline_style( 'onemall_custom_css', $output_css );
		}
		...
		*/
		if( $sticky_menu ) :		
				if( $onemall_header_style == 'style1' || $onemall_header_style == 'style'){			
			...
			** Adnvanced JS
			*/
			if( onemall_options()->getCpanelValue( 'advanced_js' ) != '' ) :
				$output .= onemall_options()->getCpanelValue( 'advanced_js' );
			endif;
		...
		$translation_text = array(
			'cart_text' 		 => esc_html__( 'Add To Cart', 'onemall' ),
			'compare_text' 	 => esc_html__( 'Compare', 'onemall' ),
			'wishlist_text'  => esc_html__( 'WishList', 'onemall' ),
			'quickview_text' => esc_html__( 'QuickView', 'onemall' ),
			'ajax_url' => admin_url( 'admin-ajax.php', 'relative' ), 
			'redirect' => get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ),
			'message' => esc_html__( 'Please enter your usename and password', 'onemall' ),
		);
		
		wp_localize_script( 'onemall_custom_js', 'custom_text', $translation_text ); 
		wp_enqueue_script( 'onemall_custom_js', get_template_directory_uri() . '/js/main.js', array(), null, true );
		wp_add_inline_script( 'onemall_custom_js', $output );
	
}
add_action( 'wp_enqueue_scripts', 'onemall_advanced', 101 );


/**
* Set and Get view count
**/
function onemall_getPostViews($postID){    
...
function onemall_setPostViews($postID) {
	$count_key = 'post_views_count';
	$count = get_post_meta($postID, $count_key, true);
	if($count==''){
			$count = 0;
...
*/
add_action( 'wp_head', 'onemall_create_postview' );
function onemall_create_postview(){
	if( is_single() || is_singular( 'product' ) ) :
		onemall_setPostViews( get_the_ID() );
	endif;
}
...
** Onemall Logo
*/
function onemall_logo(){
	$scheme_meta = get_post_meta( get_the_ID(), 'scheme', true );
	$scheme 		 = ( $scheme_meta != '' && $scheme_meta != 'none' ) ? $scheme_meta : onemall_options()->getCpanelValue( 'scheme' );
	$meta_img_ID = get_post_meta( get_the_ID(), 'page_logo', true );
	$meta_img 	 = ( $meta_img_ID != '' ) ? wp_get_attachment_image_url( $meta_img_ID, 'full' ) : '';
	$mobile_logo = onemall_options()->getCpanelValue( 'mobile_logo' );
	$logo_select = ( onemall_mobile_check() && $mobile_logo != ''  ) ? $mobile_logo : onemall_options()->getCpanelValue( 'sitelogo' );
...
add_action( 'save_post', 'onemall_product_save_meta', 10, 1 );
function post_init(){
	add_meta_box("onemall_product_meta", esc_html__( 'Recommend Product:', 'onemall' ), "onemall_product_meta", "product", "normal", "low");
	add_meta_box("onemall_product_video_meta", esc_html__( 'Featured Video Product', 'onemall' ), "onemall_product_video_meta", "product", "side", "low");
	add_meta_box("onemall_location_product_meta", esc_html__( 'Location Product:', 'onemall' ), "onemall_location_product_meta", "product", "normal", "low");
}	
function onemall_product_meta(){
	global $post;
	$recommend_product = get_post_meta( $post->ID, 'recommend_product', true );
	$newproduct 	   = get_post_meta( $post->ID, 'newproduct', true );
?>
	<p><label><b><?php esc_html_e( 'Recommend Product:', 'onemall' ) ?></b></label> &nbsp;&nbsp;
	<input type="checkbox" name="recommend_product" value="1" <?php echo checked( $recommend_product, 1 ) ?> /></p>
	
	<p><label><b><?php esc_html_e( 'New Product', 'onemall' ) ?></b></label> &nbsp;&nbsp;
		<input type="number" name="newproduct" value="<?php echo esc_attr( $newproduct ) ?>"/>
		<span class="p-description"><?php echo esc_html__( 'Set day for the new product label from the date publish product.', 'onemall' ); ?></span>
	</p>
<?php }

function onemall_location_product_meta(){
	global $post;
	$location_product = get_post_meta( $post->ID, 'location_product',true);
?>
	<p><label><b><?php _e('Location product', 'onemall'); ?>:</b></label>
		<input type ="text" name = "location_product" value ="<?php echo esc_attr( $location_product );?>" />
	</p>
	<?php }
	
function onemall_product_video_meta(){
	global $post;
...
?>
	<div class="featured-image">
		...
		<?php endif; ?>
		<p><input type="text" name="featured_video_product" placeholder="<?php echo esc_attr__( 'Youtube Video ID', 'onemall' ) ?>" value="<?php echo esc_attr( $featured_video_product ); ?>"/></p>
	</div>
<?php 
}

function onemall_product_save_meta( $post_id ){
...
** BLog columns
*/
function onemall_blogcol(){
	global $sw_blogcol;
	$blog_col = ( isset( $sw_blogcol ) && $sw_blogcol > 0 ) ? $sw_blogcol : onemall_options()->getCpanelValue('blog_column');
...
function onemall_trim_words( $title ){
	$title_length = intval( onemall_options()->getCpanelValue( 'title_length' ) );
	$html = '';
	if( $title_length > 0 ){
...
** Advanced Favico
*/
add_filter( 'get_site_icon_url', 'onemall_site_favicon', 10, 1 );
function onemall_site_favicon( $url ){
	if ( onemall_options()->getCpanelValue('favicon') ){
		$url = esc_url( onemall_options()->getCpanelValue('favicon') );
	}
	return $url;
}
...
** Social Link
*/
function onemall_social_link(){
	$fb_link = onemall_options()->getCpanelValue('social-share-fb');
	$tw_link = onemall_options()->getCpanelValue('social-share-tw');
	$tb_link = onemall_options()->getCpanelValue('social-share-tumblr');
	$li_link = onemall_options()->getCpanelValue('social-share-in');
	$gg_link = onemall_options()->getCpanelValue('social-share-go');
	$pt_link = onemall_options()->getCpanelValue('social-share-pi');
	$it_link = onemall_options()->getCpanelValue('social-share-instagram');

	$html = '';
	if( $fb_link != '' || $tw_link != '' || $tb_link != '' || $li_link != '' || $gg_link != '' || $pt_link != '' ):
	$html .= '<div class="onemall-socials"><ul>';
		if( $fb_link != '' ):
			$html .= '<li><a href="'. esc_url( $fb_link ) .'" title="'. esc_attr__( 'Facebook', 'onemall' ) .'"><i class="fa fa-facebook"></i></a></li>';
		endif;
		
		if( $tw_link != '' ):
			$html .= '<li><a href="'. esc_url( $tw_link ) .'" title="'. esc_attr__( 'Twitter', 'onemall' ) .'"><i class="fa fa-twitter"></i></a></li>';
		endif;
		
		if( $tb_link != '' ):
			$html .= '<li><a href="'. esc_url( $tb_link ) .'" title="'. esc_attr__( 'Tumblr', 'onemall' ) .'"><i class="fa fa-tumblr"></i></a></li>';
		endif;
		
		if( $li_link != '' ):
			$html .= '<li><a href="'. esc_url( $li_link ) .'" title="'. esc_attr__( 'Linkedin', 'onemall' ) .'"><i class="fa fa-linkedin"></i></a></li>';
		endif;
		
		if( $it_link != '' ):
			$html .= '<li><a href="'. esc_url( $it_link ) .'" title="'. esc_attr__( 'Instagram', 'onemall' ) .'"><i class="fa fa-instagram"></i></a></li>';
		endif;
		
		if( $gg_link != '' ):
			$html .= '<li><a href="'. esc_url( $gg_link ) .'" title="'. esc_attr__( 'Google+', 'onemall' ) .'"><i class="fa fa-google-plus"></i></a></li>';
		endif;
		
		if( $pt_link != '' ):
			$html .= '<li><a href="'. esc_url( $pt_link ) .'" title="'. esc_attr__( 'Pinterest', 'onemall' ) .'"><i class="fa fa-pinterest"></i></a></li>';
		endif;
	$html .= '</ul></div>';
	endif;
...
*/
function onemall_breadcrumb_title(){
	$maintaince_attr = ( onemall_options()->getCpanelValue( 'bg_breadcrumb' ) != '' ) ? 'style="background: url( '. esc_url( onemall_options()->getCpanelValue( 'bg_breadcrumb' ) ) .' )"' : '';
?>
	<div class="listings-title" <?php echo $maintaince_attr; ?>>
		<div class="container">
			<div class="wrap-title">
				...
				<?php 	
					if( class_exists( 'WooCommerce' ) ) :
						( is_woocommerce() ) ? woocommerce_page_title() : onemall_title();			
					else: 
						onemall_title();	
					endif;
				?>
				...
				<?php if (!is_front_page() ) { ?>
					<div class="bread">
						<?php
							...
							if (function_exists('onemall_breadcrumb')){
								onemall_breadcrumb('<div class="breadcrumbs theme-clearfix"><div class="container">', '</div></div>');
							} 

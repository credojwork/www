...
if ( ! class_exists( 'WC_Currency_Converter' ) ) {
	...
	class WC_Currency_Converter {
		...
		public function __construct() {
			$this->settings = array(
				array(
					'name' => esc_html__( 'Open Exchange Rate API', 'onemall' ),
					'type' => 'title',
					'desc' => '',
					'id'   => 'woocommerce-currency-converter-widget'
				),
				array(
					'name' => esc_html__( 'App Key', 'onemall' ),
					'desc' => sprintf( __( '(optional) If you have an <a href="%s">Open Exchange Rate API app ID</a>, enter it here.', 'onemall' ), 'https://openexchangerates.org/signup' ),
		...
		public function admin_notice_wrong_key() {
			?>
			<div class="error">
				<p><?php esc_html_e( 'WooCommerce Currency Converter: Incorrect key entered!', 'onemall' ); ?></p>
			</div>
		<?php
		}
		...
		public function load_plugin_textdomain() {
			load_plugin_textdomain( 'onemall', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
		}
		...
		public function get_converter_form( $instance, $echo = true ) {
			...
			$html = '<form method="post" id="currency_converter">' . "\n";
			...
			if ( $currencies ) {
				...
				if ( ! empty( $instance['currency_display'] ) && 'select' === $instance['currency_display'] ) {
					...
					if ( ! empty( $instance['show_reset'] ) ) {
						$html .= ' <a href="#" class="wc-currency-converter-reset reset">' . esc_html__( 'Reset', 'onemall' ) . '</a>';
					}
					...
					if ( ! empty( $instance['show_reset'] ) ) {
						$html .= '<li><a href="#" class="wc-currency-converter-reset reset">' . esc_html__( 'Reset', 'onemall' ) . '</a></li>';
					}
		...
		public function enqueue_assets() {
			...
			$wc_currency_converter_params = array(
				...
				'i18n_oprice'            => esc_html__( 'Original price:', 'onemall' ),

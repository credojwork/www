...
if ( is_admin()){
	add_filter('onemall-opts-rights', 'add_field_rights', 10, 2);
}

function onemall_options(){
	global $onemall_options;
	return $onemall_options;
}

$add_query_vars = array();
function onemall_query_vars( $qvars ){
...
if (!is_admin()){
	add_filter('query_vars', 'onemall_query_vars');
	add_action('parse_request', 'onemall_parse_request');
}
?>
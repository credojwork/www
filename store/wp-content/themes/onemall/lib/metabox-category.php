...
*/
	...
	$onemall_taxies = onemall_options()->getCpanelValue( 'tax_select' );
	/* Add Custom field to category product */
	if( !empty( $onemall_taxies ) ){
		foreach( $onemall_taxies as $onemall_tax ){
			add_action( $onemall_tax . '_add_form_fields', 'onemall_category_fields', 200 );
			add_action( $onemall_tax . '_edit_form_fields', 'onemall_edit_category_fields', 200 );
		}
		add_action( 'created_term', 'onemall_save_category_fields', 10, 3 );
		add_action( 'edit_terms', 'onemall_save_category_fields', 10, 3 );
	}
	
	function onemall_category_fields(){
		$number  = array( 0 => esc_html__( 'Select column', 'onemall' ), 1, 2, 3, 4 );
		$sale_of = array() ;
		$sidebar = array( 
...
?>
	<tr class="form-field">
		<th scope="row" valign="top"><label for="extra1"><?php esc_html_e('Sale Of(%)','onemall'); ?></label></th>
		<td>
	...
	<div class="form-field">
		<label><?php  esc_html_e( 'Sidebar Product Layout', 'onemall' ) ?></label>
	...
	<div class="form-field">
		<label><?php  esc_html_e( 'Select column for desktop screen', 'onemall' ) ?></label>
	...
	<div class="form-field">
		<label><?php  esc_html_e( 'Select column for small desktop screen', 'onemall' ) ?></label>
	...
	<div class="form-field">
		<label><?php  esc_html_e( 'Select column for tablet screen', 'onemall' ) ?></label>
...
<?php 
	...
	function onemall_edit_category_fields( $term ){
		$number = array( 0 => esc_html__( 'Select column', 'onemall' ), 1, 2, 3, 4 );
		$sale_of = array();
		$sidebar = array( 
			'left'	=> esc_html__( 'Left Sidebar', 'onemall' ),
...
?>
	...
	<tr class="form-field">
		<th scope="row" valign="top"><label><?php  esc_html_e( 'Sidebar Product Layout', 'onemall' ) ?></label></th>
		<td>	
			<select id="term_sidebar" name="term_sidebar">
				<?php 
					foreach( $sidebar as $k => $v ){
	...
	<tr class="form-field">
		<th scope="row" valign="top"><label><?php  esc_html_e( 'Select column for desktop screen', 'onemall' ) ?></label></th>
		<td>
			<select id="term_col_lg" name="term_col_lg">
				<?php 
					foreach( $number as $k => $v ){
	...
	<tr class="form-field">
		<th scope="row" valign="top"><label><?php  esc_html_e( 'Select column for medium desktop screen', 'onemall' ) ?></label></th>
		<td>
			<select id="term_col_md" name="term_col_md">
				<?php 
					foreach( $number as $k => $v ){
	...
	<tr class="form-field">
		<th scope="row" valign="top"><label><?php  esc_html_e( 'Select column for tablet screen', 'onemall' ) ?></label></th>
		<td>
			<select id="term_col_sm" name="term_col_sm">
				<?php 
					foreach( $number as $k => $v ){
...
<?php 
	...
	function onemall_save_category_fields( $term_id, $tt_id = '', $taxonomy = '', $prev_value = '' ){

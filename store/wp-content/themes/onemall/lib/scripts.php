...
function onemall_scripts() {	
	$scheme_meta = get_post_meta( get_the_ID(), 'scheme', true );
	$scheme = ( $scheme_meta != '' && $scheme_meta != 'none' && is_page() ) ? $scheme_meta : onemall_options()->getCpanelValue('scheme');
	...
	if ( !is_admin() ){			
		...
		wp_enqueue_style('onemall_css', $app_css, array(), null);	
		...
		if( is_rtl() || $onemall_direction == 'rtl' ){
			wp_enqueue_style('rtl_css', get_template_directory_uri() . '/css/rtl.css', array(), null);
		}
		wp_enqueue_style('onemall_responsive_css', get_template_directory_uri() . '/css/app-responsive.css', array(), null);
		
		/* Load style.css from child theme */
		if (is_child_theme()) {
			wp_enqueue_style('onemall_child_css', get_stylesheet_uri(), false, null);
		}
	...
	$overflow_text = array(
		'more_text' => esc_html__( 'More...', 'onemall' ),
		'more_menu'	=> onemall_options()->getCpanelValue( 'more_menu' )
	);
	...
	*/
	if( !is_user_logged_in() && onemall_options()->getCpanelValue('maintaince_enable') ){ 
		$output = '';
		$countdown = onemall_options()->getCpanelValue('maintaince_date');
		if( $countdown != '' ):
			$output .= 'jQuery(function($){
			"use strict";
			function onemall_check_height(){
	...
	*/
	if( onemall_mobile_check() ) :
		...
		if( !onemall_options()->getCpanelValue( 'mobile_jquery' ) ) :
		...
		wp_dequeue_script( 'onemall_megamenu' );
		wp_dequeue_script( 'moneyjs' );
		wp_dequeue_script( 'onemall' );
...
add_action('wp_enqueue_scripts', 'onemall_scripts', 100);

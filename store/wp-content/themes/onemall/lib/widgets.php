 ...
 */
function onemall_unregister_default_widgets() {
...
add_action('widgets_init', 'onemall_unregister_default_widgets', 11);
 
/*
** Register Sidebar and Widgets
*/
function onemall_widgets_init() {
	
	// Sidebars
	global $onemall_widget_areas;
	$onemall_widget_areas = onemall_widget_setup_args();
	if ( count($onemall_widget_areas) ){
		foreach( $onemall_widget_areas as $sidebar ){
			$sidebar_params = apply_filters('onemall_sidebar_params', $sidebar);
			register_sidebar($sidebar_params);
		}
	}

	// Widgets
	register_widget('Onemall_Social_Widget');
	register_widget('Onemall_Top_Widget');
}
add_action('widgets_init', 'onemall_widgets_init');
...
class Onemall_Top_Widget extends Onemall_Widget{

	function __construct(){
		$widget_ops = array('classname' => 'sw_top', 'description' => esc_html__('SW top header widget', 'onemall'));
		parent::__construct('sw_top', esc_html__('SW Top Widget', 'onemall'), $widget_ops);
	}
}

class Onemall_Social_Widget extends WP_Widget{

	function __construct(){
		$widget_ops = array('classname' => 'sw_social', 'description' => esc_html__('SW Social Networks', 'onemall'));
		parent::__construct('sw_social', esc_html__('SW Social', 'onemall'), $widget_ops);
	...
	function form($instance){ 
		...
		?>
<p>
	<label for="<?php echo esc_attr( $this->get_field_id('title') ); ?>"><?php esc_html_e('Title: ', 'onemall'); ?>
...
<?php 
		...
		foreach ($socials as $j => $social){
			...
			?>
<p>
	<strong><?php echo esc_html__( 'Social', 'onemall' ). ' ' .$j+1; ?>
	</strong><br>
	<?php if (key_exists('icon', $social)){?>
	<label for="<?php echo esc_attr( $id_j_icon ); ?>"><?php esc_html_e('fa-* | title', 'onemall')?>
	...
	<?php }?>
	<label for="<?php echo esc_attr( $id_j_link ); ?>"><?php esc_html_e('Link ', 'onemall')?> </label>
...
<?php
		...
		// onemall fields for add new social network
		...
		?>
<p>
	<strong><?php esc_html_e( 'Add a new social network', 'onemall' ) ?></strong><br> <label
		for="<?php echo esc_attr( $id_i_icon ); ?>"><?php esc_html_e('Classname for Icon or Title', 'onemall'); ?>
	</label> <input class="widefat" id="<?php echo esc_attr( $id_i_icon ); ?>"
		name="<?php echo esc_attr( $name_i_icon ); ?>" type="text" value=""
		placeholder="<?php esc_attr_e( 'Enter Font Awesome icon or Title', 'onemall' ) ?>" /> <span><?php esc_html_e( 'If using as
		icon, please choose class name in Font Awesome. This is required.', 'onemall' ) ?></span>
	<label for="<?php echo esc_attr( $id_i_link ); ?>"><?php esc_html_e('Link ', 'onemall')?> </label>
...
class Onemall_Widgets{
	...
	protected $fields = array(
			'onemall_before_widget' => 'Before Widget',
			'onemall_after_widget'  => 'After Widget',
			'onemall_before_title'  => 'Before Title',
			'onemall_after_title'   => 'After Title',
			'onemall_display_conditions' => 'Display Conditions',
	...
	public function widget_display_callback( $instance, $widget, $args ){		
		...
		if ( onemall_check_wdisplay($widget_display) == false ) return false;
	...
	public function in_widget_form($widget, $r = null, $instance = array() ){
		...
		$widget_advanced = onemall_options()->getCpanelValue('widget-advanced');
		...
		<div class="advanced-opt <?php echo $adoptions_on_class; ?>">
			<div class="advanced-opt-controls" >
				<label class="button toggle" for="<?php echo esc_attr( $widget->get_field_id('adoptions') ); ?>"><?php esc_html_e( 'Advanced Options', 'onemall' ) ?>
			...
			<div class="advanced-opt-pane">
				<div class="advanced-opt-pane-inner">
					<div class="pane-content">
						<div class="pane-left">
							<p>
								<label for="<?php echo esc_attr( $widget->get_field_id('widget_style') ); ?>"><?php esc_html_e( 'Widget Style', 'onemall' ) ?>
	...
	private function getDisplaySelectOptions(){
		if ( is_null($this->display_select_tpl) ){
			$this->display_select_options = $display_select_options = array(
				'all' => esc_html__('All', 'onemall'),
				'if_selected' => esc_html__('Only on selected', 'onemall'),
				'if_no_selected' => esc_html__('Except selected', 'onemall')
			);
			$html = '';
			$html .= '<p>';
			$html .= '<label for="id-display-select">'. esc_html__( 'Widget Display', 'onemall' ) .  '</label>';
	...
	private function getDisplayCheckboxUsers(){
		if ( is_null($this->display_checkbox_users_tpl) ){
			$this->display_checkbox_users = $display_checkbox_users = array(
				'login' => esc_html__('Show only for login users' , 'onemall'),
				'logout' => esc_html__('Show only for logout users' , 'onemall')
			);
	...
	private function getDisplayLanguage(){
		...
		$html = '';
		$html .= '<p>';
		$html .= '<label for="id-display-language">'. esc_html__( 'Language Display', 'onemall' ) .  '</label>';
	...
	private function getDisplayCheckboxGeneral(){
		if ( is_null($this->display_checkbox_general_tpl) ){
			$this->display_checkbox_general = $display_checkbox_general = array(
					'home' => esc_html__('Blog posts', 'onemall'),
					'archive' => esc_html__('Archive page', 'onemall'),
					'date' => esc_html__('Archive Date', 'onemall'),
					'category' => esc_html__('Category archive', 'onemall'),
					'tag' => esc_html__('Tag archive', 'onemall'),
					'author' => esc_html__('Author archive', 'onemall'),
					'search' => esc_html__('Search page', 'onemall'),
					'single' => esc_html__('Single post', 'onemall'),
					'page' => esc_html__('Page template', 'onemall'),
					'404' => esc_html__('Page 404', 'onemall')
				);
			$html = '';
				...
				if(!isset($menu) || $menu != NULL || $menu != "")
					{
						...
						if (count($items) > 0) {
							$menu_checkbox = new Onemall_Menu_Checkbox( $menu->slug);
	...
	protected function getWidgetStyles(){
		...
		if ( is_null($this->styles) ){
			$tmp = array();
			if ( $_core_styles = $this->scandir(ONEMALL_DIR.'/widgets/_styles', 'php') )
	...
	protected function loadStyle( $style = '' ){
		...
		if ( !empty($style) ){
			...
			$_core_style = ONEMALL_DIR.'/widgets/_styles/'.$style.'.php';
